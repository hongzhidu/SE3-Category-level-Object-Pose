...
def save_nocs_model_to_file(obj_model_dir, save_dir):
    ...
    #     with open(os.path.join(save_dir, 'camera_{}.pkl'.format(subset)), 'wb') as f:
    ...
    for subset in ['real_train', 'real_test']:
        ...
        with open(os.path.join(save_dir, '{}.pkl'.format(subset)), 'wb') as f:
            cPickle.dump(real, f)
    # save mug_meta information for re-labeling
    # with open(os.path.join(save_dir, 'mug_meta.pkl'), 'wb') as f:
    #     cPickle.dump(mug_meta, f)

if __name__ == '__main__':
    obj_model_dir = '/media/ubuntu/data/6DoF/Datasets/obj_models'
    save_dir = '/media/ubuntu/data/6DoF/Datasets/mydata/obj_models_new'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    # Save ground truth models for training deform network
    save_nocs_model_to_file(obj_model_dir, save_dir)
