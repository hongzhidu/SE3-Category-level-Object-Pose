...
class PoseDataset(data.Dataset):
    def __init__(self, source=None, mode='test',
                 n_pts=1024, img_size=256, data_dir = None):
        '''
    ...
    def __getitem__(self, index):
        ...
        data_dict['sym_info'] = torch.as_tensor(sym_infos.astype(float)).contiguous()
        data_dict['mean_shape'] = torch.as_tensor(mean_shapes, dtype=torch.float32).contiguous()
        data_dict['pcl_in'] = torch.as_tensor(point_clouds, dtype=torch.float32).contiguous()
        data_dict['pcl_comp'] = points
