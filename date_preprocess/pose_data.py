...
def create_img_list(data_dir, save_dir):
    ...
    #     if not os.path.exists(os.path.join(save_dir, 'CAMERA')):
    #         os.makedirs(os.path.join(save_dir, 'CAMERA'))
    ...
    for subset in ['real_train', 'real_test']:
        ...
        for folder in folder_list:
            ...
            for img_full_path in img_paths:
                ...
                img_list.append(img_path)
        if not os.path.exists(os.path.join(save_dir, 'Real')):
            os.makedirs(os.path.join(save_dir, 'Real'))
...
def annotate_camera_train(data_dir, save_dir):
    """ Generate gt labels for CAMERA train data. """
    camera_train = open(os.path.join(save_dir, 'CAMERA', 'train_list_all.txt')).read().splitlines()
    intrinsics = np.array([[577.5, 0, 319.5], [0, 577.5, 239.5], [0, 0, 1]])
    # meta info for re-label mug category
    with open(os.path.join(save_dir, 'obj_models/mug_meta.pkl'), 'rb') as f:
    ...
    for img_path in tqdm(camera_train):
        ...
        save_path = os.path.join(save_dir, 'CAMERA', img_path)
    ...
    with open(os.path.join(save_dir, 'CAMERA/train_list.txt'), 'w') as f:
        for img_path in valid_img_list:
            f.write("%s\n" % img_path)


def annotate_real_train(data_dir, save_dir):
    """ Generate gt labels for Real train data through PnP. """
    real_train = open(os.path.join(save_dir, 'Real/real_train_list_all.txt')).read().splitlines()
    ...
    for img_path in tqdm(real_train):
        ...
        save_path = os.path.join(save_dir, 'Real', img_path)
        ...
        valid_img_list.append(img_path)
    # write valid img list to file
    with open(os.path.join(save_dir, 'Real/real_train_list.txt'), 'w') as f:
        for img_path in valid_img_list:
            f.write("%s\n" % img_path)
...
def annotate_test_data(data_dir):
    ...
    camera_val = open(os.path.join(save_dir, 'CAMERA', 'val_list_all.txt')).read().splitlines()
    real_test = open(os.path.join(save_dir, 'Real', 'real_test_list_all.txt')).read().splitlines()
    ...
    for path in model_file_path:
        with open(os.path.join(save_dir, path), 'rb') as f:
    ...
    for source, img_list, intrinsics, subset in subset_meta:
        ...
        for img_path in tqdm(img_list):
            ...
            nocs_dir = '/media/ubuntu/data/6DoF/Datasets/mydata/deformnet_eval/nocs_results'
            if source == 'CAMERA':
            ...
            save_path = os.path.join(save_dir, source, img_path)
            ...
            valid_img_list.append(img_path)
        # write valid img list to file
        with open(os.path.join(save_dir, source, subset+'_list.txt'), 'w') as f:
            for img_path in valid_img_list:
                f.write("%s\n" % img_path)


if __name__ == '__main__':
    data_dir = '/media/ubuntu/data/6DoF/Datasets'
    save_dir = '/media/ubuntu/data/6DoF/Datasets/mydata'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    # create list for all data
    create_img_list(data_dir, save_dir)
    # annotate dataset and re-write valid data to list
    # annotate_camera_train(data_dir, save_dir)
    # annotate_real_train(data_dir, save_dir)
